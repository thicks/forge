export function generateWorkosLoginPage(appName: string): string {
...
export default async function LoginPage() {
	// If WorkOS is configured, redirect to WorkOS hosted login
	if (isWorkOSConfigured) {
		const signInUrl = await getSignInUrl();
		redirect(signInUrl);
	}

	// Otherwise show dev login form
	return <DevLoginForm appName="${appName}" />;
}
`;
}

export function generateDevLoginForm(appName: string): string {
	return `"use client";
...
export function DevLoginForm({ appName }: DevLoginFormProps) {
				...
				<div className="p-6 pt-0">
					...
					<div className="mt-6 rounded-md border border-amber-200 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950">
						<p className="text-xs text-amber-800 dark:text-amber-200">
							<span className="font-medium">Dev mode:</span> Username and password are both{" "}
							<code className="rounded bg-amber-100 px-1 py-0.5 font-mono dark:bg-amber-900">{appName}</code>
						</p>
					</div>
				</div>
			</div>
