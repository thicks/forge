...
export function generateStandalonePackageJson(
	...
	// Add postgres docker start script and wire it into dev
	if (db === "postgres") {
		scripts["db:start"] = "./scripts/db-start";
		scripts.dev = "pnpm db:start && next dev";
	}

	// Add supabase scripts when using supabase db
	if (db === "supabase") {
	...
	// cn utility dependencies (clsx + tailwind-merge)
